        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          >
            <div className="relative mb-8 aspect-auto sm:aspect-[4/3] w-full rounded-2xl overflow-hidden shadow-xl max-w-md mx-auto lg:mx-0 group">
              ...
              />
              <div className="absolute inset-0 bg-neon-cyan/20 mix-blend-multiply"></div>
            </div>
          ...
          >
            ...
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-px before:bg-gradient-to-b before:from-transparent before:via-neon-purple/50 before:to-transparent">
              
              {education.map((edu, index) => (
                <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-neon-cyan bg-dark-bg text-neon-cyan shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-[0_0_10px_rgba(0,240,255,0.5)] relative z-10 transition-all duration-300 group-hover:bg-neon-cyan group-hover:text-dark-bg group-hover:shadow-[0_0_20px_rgba(0,240,255,0.8)]">
                    <div className="w-2.5 h-2.5 bg-neon-cyan rounded-full group-hover:bg-dark-bg"></div>
                  </div>
                  
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl glass-panel glass-panel-hover">
                    <div className="flex items-center justify-between space-x-2 mb-1">
                      <div className="font-bold text-gray-900 dark:text-white">{edu.degree}</div>
                    </div>
                    <div className="text-neon-cyan font-medium text-sm mb-2">{edu.duration}</div>
                    <div className="text-gray-700 dark:text-gray-300 font-medium">{edu.institution}</div>
                    <div className="text-sm text-neon-cyan mt-2 font-semibold bg-neon-cyan/10 border border-neon-cyan/20 inline-block px-2 py-1 rounded">
                      {edu.score}
                    </div>
                  </div>
                </div>
